...
def getNewVariants(smcounter_vcf, mutect2_vcf, varscan2_vcf, variants_list, detail_table, rate, sample, depth_true):
    #statistics new vcf
    for l1 in open(smcounter_vcf, 'r'):
        if not l1.startswith('#'):
            chrom, pos, xxx, ref, alt, qual, fil, info, fmt, freq = l1.split('\t')
            ...
            if (chrom, pos, ref, alt) in variants_list:
                detail_table.write(','.join([sample, 'smcounter', str(rate), chrom, pos, ref, alt, dp, fr]))
                ref_dp, alt_dp = dp.split(';')
    ...
    for l2 in open(mutect2_vcf, 'r'):
        if not l2.startswith('#'):
            ...
            fr = freq.split(':')[2]
            if (chrom, pos, ref, alt) in variants_list:
                detail_table.write(','.join([sample, 'mutect2', str(rate), chrom, pos, ref, alt, dp, fr+'\n']))
                ref_dp, alt_dp = dp.split(';')
    ...
    for l3 in f3.readlines():
        ...
        if (chrom, pos, ref, alt) in variants_list:
            detail_table.write(','.join([sample, 'varscan2', str(rate), chrom, pos, ref, alt, dp, fr+'\n']))
            ref_dp, alt_dp = dp.split(';')
...
def main():
    ...
    detail_table = open(path + '_detail_table.csv', 'w')
    stats_table = open(path + '_stats.csv', 'w')
        ...
        for rate in [0.99, 0.95, 0.85, 0.65, 0.5]:
            ...
            for variant, software_to_option in depth_true.items():
                ...
                for software, option in software_to_option.items():
                    ...
                    theory_ref = int(depth_theory[variant][software]*rate)
                    theory_alt = int(depth_theory[variant][software]*(1-rate))
