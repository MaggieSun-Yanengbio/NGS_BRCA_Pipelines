...
def trim_read_pairs(read1, read2, trimmed1, trimmed2, min_read_len, common_seq, stats_file,
    ...
    for r1, r2 in zip(read_fq(read1,logger_trim_process,logger_trim_errors), 
        ...
        if r1[0][0] != '@' or r2[0][0] != '@':
            ...
            start_common = r2[1].find(common_seq)
            if start_common < 12:
                num_error_reads += 1
                logger_trim_process.info("Error barcode/common seqs:"+str(start_common)+"\n\t"
                    +'\t'.join(r1)+'\n\t'+'\t'.join(r2)+'\n')
            else:
                umi = r2[1][(start_common-12):start_common]+';'+r2[3][(start_common-12):start_common]
                seq2 = r2[1][(start_common+11):]
                qual2 = r2[3][(start_common+11):]
                h1 = r1[0].strip()+' '+umi+'\n'
                h2 = r2[0].strip()+' '+umi+'\n'
...
def main():
    ...
    common_seq = 'ATTGGAGTCCT'
    logger_trim_process, logger_trim_errors = store_logs(log_dir)
    trim_read_pairs(read1, read2, trimmed1, trimmed2, min_read_len, common_seq, stats_file,
        logger_trim_process, logger_trim_errors)
